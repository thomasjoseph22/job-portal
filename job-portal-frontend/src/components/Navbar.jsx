...
const Navbar = ({ hasUnviewedJobs }) => {
  ...
  const { unviewedJobs, setUnviewedJobs } = useContext(JobContext); // Access the setter
  ...
  const handleLogout = () => {
    ...
    setUnviewedJobs(0); // Reset notifications
    navigate('/');
  };
  ...
  const closeSidebar = () => {
    setSidebarOpen(false); // Explicitly close the sidebar
  };
    ...
    <nav className="navbar">
      ...
      {isSidebarOpen && (
        <div className="sidebar">
          ...
          <Link to="/job-alerts" className="sidebar-link" onClick={closeSidebar}>Job Alerts</Link>
          <Link to="/applied-job" className="sidebar-link" onClick={closeSidebar}>Applied Jobs</Link>
          {isApplicant && (
            <button
              onClick={() => {
                handleLogout();
