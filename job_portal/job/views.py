...
class JobViewedView(APIView):
    def post(self, request, job_id):
        ...
        job_view, created = JobView.objects.get_or_create(user=user, job=job)
        
        # If the JobView exists, mark it as viewed
        if not job_view.viewed:
            job_view.viewed = True
            job_view.save()

        return Response({"viewed": True})


class MarkJobViewed(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request, job_id):
        return self._mark_as_viewed(request, job_id)
    
    def patch(self, request, job_id):
        return self._mark_as_viewed(request, job_id)

    def _mark_as_viewed(self, request, job_id):
        try:
            job = Job.objects.get(id=job_id)
            user = request.user

            # Mark the job as viewed for the user
            job_view, created = JobView.objects.get_or_create(user=user, job=job)
            job_view.viewed = True
            job_view.save()

            # Return the updated job data including the 'viewed' status
...
class JobAlertsView(APIView):
    permission_classes = [IsAuthenticated]

    def get_viewed(self, obj):
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return JobView.objects.filter(job=obj, user=request.user, viewed=True).exists()
        return False
...
def get_user_details(request):
    ...
    # Assuming the user model has a 'company_name' field or related 'Profile' model
    company_name = getattr(user, 'company_name', None)  # or user.profile.company_name if using a Profile model
    
    return Response({
        'username': user.username,
        'email': user.email,
        'company_name': company_name  # Return the company name as part of the user details
    })
